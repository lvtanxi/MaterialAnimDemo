...
public class ATLoginButton extends View {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (widthMode == MeasureSpec.AT_MOST || widthMode == MeasureSpec.UNSPECIFIED) {
            widthSize = getApplyDimension(DE_W);
            widthMeasureSpec = MeasureSpec.makeMeasureSpec(widthSize, MeasureSpec.EXACTLY);
        }

        if (heightMode == MeasureSpec.AT_MOST || heightMode == MeasureSpec.UNSPECIFIED) {
            heightSize = getApplyDimension(DE_H);
    ...
    private int getApplyDimension(float val) {
    ...
    private void drawTextDesc(Canvas canvas, String textDesc) {
        if (null != canvas && null != textDesc) {
            Rect rect = new Rect();//测量字体宽度
            textPaint.getTextBounds(textDesc, 0, textDesc.length(), rect);
            canvas.drawText(textDesc, getMeasuredWidth() / 2, getMeasuredHeight() / 2 + rect.height() / 2, textPaint);
        }
    }
